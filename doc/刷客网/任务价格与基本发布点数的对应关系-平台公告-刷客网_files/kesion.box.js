...
function KesionPopup(BgColor,MsgBorder,ShowBackground,TitleCss)
{
 ...
 this.FadeInTime=400;        //淡入效果时间，不想有效果输入0
 this.FadeOutTime=400;       //淡出效果时间，不想有效果输入0
	  ...
	  // if (bHeight<parseInt(document.body.scrollHeight)) bHeight=parseInt(document.body.scrollHeight);
	   ...
	   if (this.QuoteByAdmin==true){
		   ...
		   if (this.ShowClose==true){
			mesW.innerHTML="<div style='border-bottom:#eee 1px solid;padding:3px;font-weight:bold;text-align:left;font-size:12px;"+this.TitleCss+"'><table cellpadding='0' cellspacing='0' width='100%' height='100%'><tr onmousedown=MDown(mesWindow)><td><b>"+wTitle+"</b></td><td style='width:41px;'><a href='javascript:closeWindow();' id='closeBtn' style='color:#ff6600'>X关闭</a></td></tr></table></div><div style='font-size:12px;' id='mesWindowContent'>"+content+"</div><div class='mesWindowBottom'></div>";
		   }else{
